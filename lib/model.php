...
// thêm mới hoặc sửa chữa database
...
function deletefile($table, $id){
    $id     = intval($id);
    $temp   = get_a_record($table, $id);
    $img1   = 'public/upload/images/'.$temp['Image1'];
    if (is_file($img1)) {
        unlink($img1);
    }

    $img2   = 'public/upload/images/'.$temp['Image2'];
    if (is_file($img2)) {
        unlink($img2);
    }

    $img3   = 'public/upload/images/'.$temp['Image3'];
    if (is_file($img3)) {
        unlink($img3);
    }

    $img4   = 'public/upload/images/'.$temp['Image4'];
    if (is_file($img4)) {
        unlink($img4);
    }
